...
class Warsel_model extends CI_Model {
    ...
    function where_warsel_maps() {
        $ses_kecamatan_id = @$_SESSION['ses_kecamatan_id'];
        $ses_kelurahan_id = @$_SESSION['ses_kelurahan_id'];
        //
        $sql_where = "";
        if($ses_kecamatan_id != '')  $sql_where .= " AND a.warsel_alamat_kecamatan_id LIKE '%$ses_kecamatan_id%'";
        if($ses_kelurahan_id != '')  $sql_where .= " AND a.warsel_alamat_desa_id LIKE '%$ses_kelurahan_id%'";
        return $sql_where;
    }
    ...
    function get_all_warsel() {
        $sql_where = $this->where_warsel_maps();
        //
        $sql = "SELECT 
                    a.*, b.wilayah_nm as kecamatan_nm, c.wilayah_nm as desa_nm
                FROM trx_warsel a 
