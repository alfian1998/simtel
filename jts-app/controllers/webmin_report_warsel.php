...
class Webmin_Report_Warsel extends CI_Controller{
	...
	function ajax($id=null) {
		if($id == 'ses_kelurahan_id') {
			...
			//
			$list_desa = $this->warsel_model->get_all_desa_id($ses_kecamatan_id);
			//
			$html = '';
	...
	function index($p=1, $o=0) {	
		...
		//
		$data['form_action'] = site_url('webmin_report_warsel/filter');
		//
		$data['p'] = $p;
		$data['o'] = $o;
		...
		$data['list_warsel'] = $this->warsel_model->list_export_warsel();
		endif;
		//
		...
		$this->load->view('webmin/warsel/warsel_export_excel',$data);
	...
	function search() {
		...
		$data['filter_search'] = @$_SESSION['filter_search'];
		//
		$this->warsel_model->export_excel();
	}
