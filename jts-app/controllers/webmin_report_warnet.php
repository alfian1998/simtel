...
class Webmin_Report_Warnet extends CI_Controller{
	...
	function ajax($id=null) {
		if($id == 'ses_kelurahan_id') {
			...
			//
			$list_desa = $this->warnet_model->get_all_desa_id($ses_kecamatan_id);
			//
			$html = '';
	...
	function index($p=1, $o=0) {	
		...
		//
		$data['form_action'] = site_url('webmin_report_warnet/filter');
		//
		$data['p'] = $p;
		$data['o'] = $o;
		...
		$data['list_warnet'] = $this->warnet_model->list_export_warnet();
		endif;
		//
		...
		$this->load->view('webmin/warnet/warnet_export_excel',$data);
	...
	function search() {
		...
		$data['filter_search'] = @$_SESSION['filter_search'];
		//
		$this->warnet_model->export_excel();
	}
