...
class Webmin_Jml_Menara extends CI_Controller{
	...
	function index($p=1, $o=0) {	
		...
		//
		$data['form_action'] = site_url('webmin_jml_menara/filter');
		//
		$data['p'] = $p;
		$data['o'] = $o;
		...
		$this->load->view('webmin/menara/jml_menara',$data);
	...
	function search() {
		...
		if ($data['ses_jenis_laporan'] == '1') {
			$this->menara_model->export_excel_jumlah_per_kecamatan();
		}elseif($data['ses_jenis_laporan'] == '2'){
			$this->menara_model->export_excel_jumlah_per_kelurahan();
		}
	}
