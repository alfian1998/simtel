<script type="text/javascript">
	$(function() {
		...
		$('#ses_tgl_pendataan, #ses_kecamatan').bind('change',function() {
	        $('#form-search').attr('action','<?=site_url("web/search/warnet")?>').submit();
	    });
	    $('#ses_tgl_pendataan_mobile, #ses_kecamatan_mobile').bind('change',function() {
	        $('#form-search_mobile').attr('action','<?=site_url("web/search/warnet")?>').submit();
	    });
	});
</script>
<div id="landing_2">
						...
						<div class="widget-body">
							<div class="row-fluid">	
								<div class="span12">
									...
									<form name="form-search" id="form-search" method="post" action="<?=site_url('web/search/warnet')?>">
									<table width="100%">
									<tr>
										<td width="8%">
										...
										<td width="20%" align="right" valign="top">
											...
											<a href="<?=site_url('web/location/warnet')?>" class="btn btn-danger" title="Refresh Pencarian ..." style="margin-bottom: 10px;	margin-left: 0px;"><i class="icon-refresh "></i></a>
										</td>
									</tr>
									</table>
									</form>
									...
									<form name="form-search" id="form-search_mobile" method="post" action="<?=site_url('web/search/warnet')?>">
									...
									<tbody>
										...
										<tr>
											...
											<td class="center">
												<a href="<?=site_url("web/detail/warnet/$p/$o/$row[warnet_id]")?>" class="btn btn-primary icon-href" style="padding-bottom: 5px; padding-top: 5px; padding-right: 8px; padding-left: 8px;"><i class="fa fa-bars"></i></a>
											</td>
									...
									<div class="pagination center">
										<ul>
											<?php if($paging->start_link): ?>
							                    <li><a href="<?=site_url("web/warnet/$paging->c_start_link/$o") ?>">First</a></li>
							                <?php endif; ?>
							                <?php if($paging->prev): ?>
							                    <li><a href="<?=site_url("web/warnet/$paging->prev/$o") ?>">Prev</a></li>
							                <?php endif; ?>

							                <?php for($i = $paging->c_start_link; $i <= $paging->c_end_link; $i++): ?>
							                	<li <?php jecho($p, $i, "class='active'") ?>><a href="<?=site_url("web/warnet/$i/$o") ?>"><?=$i ?></a></li>
							                <?php endfor; ?>

							                <?php if($paging->next): ?>
							                    <li><a href="<?=site_url("web/warnet/$paging->next/$o") ?>">Next</a></li>
							                <?php endif; ?>
							                <?php if($paging->end_link): ?>
							                    <li><a href="<?=site_url("web/warnet/$paging->c_end_link/$o") ?>">Last</a></li>
							                <?php endif; ?>
										</ul>
									</div>
									<?php endif;?>
