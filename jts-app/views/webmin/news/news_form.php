...
$(function() {
    ...
    function __get_image(image_no, post_id, count_image) {
    	...
    	//
    	$.get('<?=site_url("webmin_news/ajax/get_image")?>?image_no='+image_no+image_var,null,function(data) {
    ...
    function __get_file(file_no, post_id, count_file) {
    	...
    	//
    	$.get('<?=site_url("webmin_news/ajax/get_file")?>?file_no='+file_no+file_var,null,function(data) {
							...
							<div class="row-fluid">	
								<div class="span12">
									<form class="row-fluid margin-none" action="<?=$form_action?>" method="post" enctype="multipart/form-data" id="form-validate">	
										<table width="100%">
										<tr>
											<td width="17%"><div class="span6">Kategori</div></td>
											<td width="85%">
												<select name="menu_id" id="menu_id">
													<?php foreach($menu_child as $child):?>
													<option value="<?=$child['menu_id']?>" <?php if($child['menu_id'] == @$main['menu_id']) echo 'selected'?>><?=$child['menu_title']?></option>
													<?php endforeach;?>
												</select>
												<a href="javascript:void(0)" class="icon-plus" data-toggle="modal" data-target="#modal-kategori" title="Tambah Kategori Berita"> Tambah Kategori</a>
											</td>
										</tr>
										<tr>
											<td><div class="span6">Judul</div></td>
											<td>
												<div class="span12">
...
<?php include("news_modal_kategori.php");?>
