...
$(function() {
    ...
    function ses_kelurahan_id(i,k) {
        $.get('<?=site_url("webmin_jml_warsel/ajax/ses_kelurahan_id")?>?ses_kecamatan_id='+i+'&ses_kelurahan_id='+k,null,function(data) {
    ...
    $('#proses').bind('click',function() {
        ...
        if(uptd_id.val() == '') {
            ...
            $('#mainform').attr('action','<?=site_url("webmin_jml_warsel/filter/jml_warsel")?>').submit();
        }        
    });
    //
    $('.row_dim').hide(); 
			...
			<div class="row-fluid">
				<div class="span12">
					...
					<div class="widget widget-heading-simple widget-body-white">
						<div class="widget-head"><h4 class="heading glyphicons list"><i></i>Resume Jumlah Data Warung Seluler (WARSEL) di Wilayah Kabupaten Kebumen</h4></div>
									...
									<form name="mainform" id="mainform" class="row-fluid margin-none" action="" method="post" enctype="multipart/form-data">	
										...
										<div class="right" style="margin-top:10px">
											...
											<?php if($filter_search == 'true'):?>
												<a href="<?=site_url('webmin_jml_warsel/search');?>" class="btn btn-success btn-icon btn-submit"><i></i> Export Excel</a>
											<?php endif; ?>
											<a href="<?=site_url('webmin_jml_warsel/location/jml_warsel')?>" class="btn btn-danger btn-icon btn-submit"><i></i> Clear</a>
										</div>
										...
										<br>
											<?php if($ses_jenis_laporan == '1'): ?>
												<div class="alert alert-success">
													<strong>Resume Jumlah Warsel Per Kecamatan</strong>
												</div>
													...
													<div class="span6">
														...
														<table style="width: 100%!important" class="table table-bordered table-primary table-striped table-vertical-center checkboxs js-table-sortable">
															...
															<tr>
																...
																<th class="center" width="30%">Nama Warsel</th>
															</tr>
														    </thead>
														    <tbody>
														    	<?php 
																...
																<tr>
																	...
																	<td class="center">
																		<?php foreach ($data['warsel_nm'] as $warsel):
																		?>
																		<?=$warsel['warsel_nm']?>,
																		<?php endforeach; ?>
																	</td>
											...
											<?php elseif($ses_jenis_laporan == '2'): ?>
												<div class="alert alert-success">
													<?php 
													if($ses_kecamatan_id != ''):
													$nama_kecamatan = $this->warsel_model->get_wilayah_by_id($ses_kecamatan_id);
													?>
														<strong>Resume Jumlah Warsel Dari Kecamatan : <?=$nama_kecamatan['wilayah_nm']?></strong>
													<?php else: ?>
														<strong>Resume Jumlah Warsel Per Kelurahan </strong>
													<?php endif; ?>
												</div>
												<div class="table-responsive">
													<div class="span6">
														    ...
														    <tbody>
																...
																<tr>
																	...
																	<tr>
																		...
																		<td class="center">
																			<?php foreach ($kel['warsel_nm'] as $warsel):
																			?>
																				<?=$warsel['warsel_nm']?>,
																			<?php endforeach; ?>
																		</td>
													...
													<div class="span6">
														...
														<?php else: ?>
															<div id="kelurahan" style="height: 18720px; "></div>
														<?php endif; ?>
													</div>
												</div>
											<?php endif; ?>
