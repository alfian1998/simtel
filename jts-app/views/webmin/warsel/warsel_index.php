<script type="text/javascript">
	$(function() {
		...
		$('#ses_tgl_pendataan, #ses_kecamatan, #ses_tahun, #ses_bulan').bind('change',function() {
	        $('#form-search').attr('action','<?=site_url("webmin_warsel/search")?>').submit();
	    });
	});
</script>
<div id="landing_2">
						...
						<div class="widget-body">
							<div class="row-fluid">	
								<div class="span12">
									...
									<?=outp_notification()?>
									<!-- Button -->
									<form name="form-search" id="form-search" method="post" action="<?=site_url('webmin_warsel/search')?>">
									<table width="100%">
									<tr>
										<td width="10%"><a href="<?=site_url('webmin_warsel/form')?>" class="btn btn-primary btn-mini" style="padding:3px 15px 3px 15px; margin-bottom:10px"><b><i class="fa fa-plus"></i> Tambah Data</b></a></td>
										<td width="7%">
										...
										<td width="4%" align="right" valign="top">
											...
											<a href="<?=site_url('webmin/location/warsel')?>" class="icon-refresh" title="Refresh Pencarian ..."></a>
										</td>
									</tr>
									</table>
									</form>
									...
									<tbody>
										...
										<tr>
											...
											<td class="center">
												<a href="<?=site_url("webmin_warsel/detail/$p/$o/$row[warsel_id]")?>" class="icon-book icon-href" title="Detail"></a>
												<a href="<?=site_url("webmin_warsel/cetak/$p/$o/$row[warsel_id]")?>" target="_blank" class="icon-print icon-href" title="Cetak PDF"></a>
												<a href="<?=site_url("webmin_warsel/delete/$p/$o/$row[warsel_id]")?>" class="icon-remove-sign icon-href" title="Delete" onclick="return confirm('Apakah anda yakin akan menghapus data ini ?')"></a>
												<a href="<?=site_url("webmin_warsel/form/$p/$o/$row[warsel_id]")?>" class="icon-pencil icon-href" title="Edit"></a>
											</td>
									...
									<div class="pagination center">
										<ul>
											<?php if($paging->start_link): ?>
							                    <li><a href="<?=site_url("webmin_warsel/index/$paging->c_start_link/$o") ?>">First</a></li>
							                <?php endif; ?>
							                <?php if($paging->prev): ?>
							                    <li><a href="<?=site_url("webmin_warsel/index/$paging->prev/$o") ?>">Prev</a></li>
							                <?php endif; ?>

							                <?php for($i = $paging->c_start_link; $i <= $paging->c_end_link; $i++): ?>
							                	<li <?php jecho($p, $i, "class='active'") ?>><a href="<?=site_url("webmin_warsel/index/$i/$o") ?>"><?=$i ?></a></li>
							                <?php endfor; ?>

							                <?php if($paging->next): ?>
							                    <li><a href="<?=site_url("webmin_warsel/index/$paging->next/$o") ?>">Next</a></li>
							                <?php endif; ?>
							                <?php if($paging->end_link): ?>
							                    <li><a href="<?=site_url("webmin_warsel/index/$paging->c_end_link/$o") ?>">Last</a></li>
							                <?php endif; ?>
										</ul>
									</div>
									<?php endif;?>
