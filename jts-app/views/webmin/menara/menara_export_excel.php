...
$(function() {
    ...
    function ses_kelurahan_id(i,k) {
        $.get('<?=site_url("webmin_report_menara/ajax/ses_kelurahan_id")?>?ses_kecamatan_id='+i+'&ses_kelurahan_id='+k,null,function(data) {
			...
			<div class="row-fluid">
				<div class="span12">
					...
					<div class="widget widget-heading-simple widget-body-white">
						...
						<!-- //Breadcrumb -->
						<div class="widget-head"><h4 class="heading glyphicons list"><i></i>Rekapitulasi Dokumen Pelaksanaan Pengawasan dan Pengendalian Menara Telekomunikasi di Wilayah Kabupaten Kebumen</h4></div>
							...
							<div class="row-fluid">	
								<div class="span12">
									<form class="row-fluid margin-none" action="<?=$form_action?>" method="post" enctype="multipart/form-data" id="form-validate">	
										...
										<tr>
						                    <td width="41%"><div class="span10">Desa/Kelurahan (Pemilik)</div></td>
						                    <td>
										...
										<div class="right" style="margin-top:10px">
											...
											<a href="webmin/location/report_menara" class="btn btn-danger btn-icon btn-submit"><i></i> Clear</a>
											<span style="margin-left: 20px;"><b>Keterangan : </b> Kolom Filter di Atas Optional</span>
											<?php if($filter_search == 'true'):?>
												<a href="<?=site_url('webmin_report_menara/search');?>" style="float: right;" class="btn btn-success btn-icon btn-submit"><i></i> Export Excel</a>
											<?php endif; ?>
										</div>
										...
										<div class="alert alert-success">
											<strong>Terdapat : <?=count($list_menara)?> Data | Untuk Export Dalam Bentuk Excel Klik Tombol Export Excel </strong>
										</div>
										...
										<table id="fixTable" class="table table-bordered table-primary table-striped table-vertical-center checkboxs js-table-sortable">
											...
											<tr>
												...
												<th class="center">Tinggi Menara</th>
												<th class="center">Jangkauan Sinyal</th>
												<th class="center">Luas Pondasi</th>
										    ...
										    <tbody>
												...
												<tr>
													...
													<td class="center"><?=$data['desa_nm']?></td>
													<td class="center"><?=$data['kecamatan_nm']?></td>
													...
													<td><?=$data['statustanah_nm']?></td>
													<td class="center"><?=$data['luastanah']?> m x <?=$data['luastanah_p']?> m x <?=$data['luastanah_l']?> m</td>
													<td class="center"><?=$data['tinggi_menara']?> m</td>
													<td class="center"><?=$data['jangkauan_sinyal']?> Km</td>
													<td class="center"><?=$data['luaspondasi']?> m x <?=$data['luaspondasi_p']?> m x <?=$data['luaspondasi_l']?> m</td>
