...
$(function() {
    ...
    function ses_kelurahan_id(i,k) {
        $.get('<?=site_url("webmin_report_warnet/ajax/ses_kelurahan_id")?>?ses_kecamatan_id='+i+'&ses_kelurahan_id='+k,null,function(data) {
			...
			<div class="row-fluid">
				<div class="span12">
					...
					<div class="widget widget-heading-simple widget-body-white">
						...
						<!-- //Breadcrumb -->
						<div class="widget-head"><h4 class="heading glyphicons list"><i></i>Rekapitulasi Dokumen Pengawasan Penyelenggara Telekomunikasi (WARNET) di Wilayah Kabupaten Kebumen</h4></div>
							...
							<div class="row-fluid">	
								<div class="span12">
									<form class="row-fluid margin-none" action="<?=$form_action?>" method="post" enctype="multipart/form-data" id="form-validate">	
										...
										<tr>
											<td width="41%"><div class="span10">Kecamatan (WARNET)</div></td>
											<td>
												<div class="span12">
										...
										<tr>
						                    <td width="41%"><div class="span10">Desa/Kelurahan (WARNET)</div></td>
						                    <td>
										...
										<div class="right" style="margin-top:10px">
											...
											<a href="webmin/location/report_warnet" class="btn btn-danger btn-icon btn-submit"><i></i> Clear</a>
											<span style="margin-left: 20px;"><b>Keterangan : </b> Kolom Filter di Atas Optional</span>
											<?php if($filter_search == 'true'):?>
												<a href="<?=site_url('webmin_report_warnet/search');?>" style="float: right;" class="btn btn-success btn-icon btn-submit"><i></i> Export Excel</a>
											<?php endif; ?>
										</div>
										...
										<div class="alert alert-success">
											<strong>Terdapat : <?=count($list_warnet)?> Data | Untuk Export Dalam Bentuk Excel Klik Tombol Export Excel </strong>
										</div>
										...
										<table id="fixTable" class="table table-bordered table-primary table-striped table-vertical-center checkboxs js-table-sortable">
										    ...
										    <tbody>
												<?php 
												$no=1;
												foreach ($list_warnet as $data): 
												?>
												<tr>
													<td class="center"><?=$no?></td>
													<td class="center"><?=convert_date($data['tgl_pendataan'])?></td>
													<td><?=$data['warnet_nm']?></td>
													<td><?=$data['warnet_alamat']?></td>
													<td class="center"><?=$data['warnet_alamat_desa_nm']?></td>
													<td class="center"><?=$data['warnet_alamat_kecamatan_nm']?></td>
													<td class="center">Kebumen</td>
													<td class="center">Jawa Tengah</td>
													<td><?=$data['warnet_telepon']?></td>
													<td class="center"><?=$data['warnet_alamat_kode_pos']?></td>
