<script type="text/javascript">
	$(function() {
		...
		$('#ses_tgl_pendataan, #ses_kecamatan, #ses_tahun, #ses_bulan').bind('change',function() {
	        $('#form-search').attr('action','<?=site_url("webmin_sinyal/search")?>').submit();
	    });
	});
</script>
<div id="landing_2">
				...
				<div class="span12">
					...
					<div class="widget widget-heading-simple widget-body-white">
					    ...
					    <!-- <ol class="breadcrumb breadcrumb-arrow">
							...
							<li class="active"><span><b>Sinyal Seluler/Telekomunikasi</b></span></li>
						</ol> -->
						<ul class="breadcrumb">
							<li><a href="<?=site_url('webmin')?>"><i class="fa fa-home"></i> Home</a></li>
							<li><a href="#">Input Data</a></li>
							<li>Sinyal Seluler</li>
						</ul>
						<!-- //Breadcrumb -->
						...
						<div class="widget-body">
							<div class="row-fluid">	
								<div class="span12">
									...
									<?=outp_notification()?>
									<!-- Button -->
									<form name="form-search" id="form-search" method="post" action="<?=site_url('webmin_sinyal/search')?>">
									<table width="100%">
									<tr>
										<td width="10%"><a href="<?=site_url('webmin_sinyal/form')?>" class="btn btn-primary btn-mini" style="padding:3px 15px 3px 15px; margin-bottom:10px"><b><i class="fa fa-plus"></i> Tambah Data</b></a></td>
										<td width="7%">
										...
										<td width="4%" align="right" valign="top">
											...
											<a href="<?=site_url('webmin/location/sinyal')?>" class="icon-refresh" title="Refresh Pencarian ..."></a>
										</td>
									</tr>
									</table>
									</form>
									...
									<tbody>
										...
										<tr>
											...
											<td class="center">
												<a href="<?=site_url("webmin_sinyal/detail/$p/$o/$row[sinyal_id]")?>" class="icon-book icon-href" title="Detail"></a>
												<a href="<?=site_url("webmin_sinyal/cetak/$p/$o/$row[sinyal_id]")?>" target="_blank" class="icon-print icon-href" title="Cetak PDF"></a>
												<a href="<?=site_url("webmin_sinyal/delete/$p/$o/$row[sinyal_id]")?>" class="icon-remove-sign icon-href" title="Delete" onclick="return confirm('Apakah anda yakin akan menghapus data ini ?')"></a>
												<a href="<?=site_url("webmin_sinyal/form/$p/$o/$row[sinyal_id]")?>" class="icon-pencil icon-href" title="Edit"></a>
											</td>
									...
									<div class="pagination center">
										<ul>
											<?php if($paging->start_link): ?>
							                    <li><a href="<?=site_url("webmin_sinyal/index/$paging->c_start_link/$o") ?>">First</a></li>
							                <?php endif; ?>
							                <?php if($paging->prev): ?>
							                    <li><a href="<?=site_url("webmin_sinyal/index/$paging->prev/$o") ?>">Prev</a></li>
							                <?php endif; ?>

							                <?php for($i = $paging->c_start_link; $i <= $paging->c_end_link; $i++): ?>
							                	<li <?php jecho($p, $i, "class='active'") ?>><a href="<?=site_url("webmin_sinyal/index/$i/$o") ?>"><?=$i ?></a></li>
							                <?php endfor; ?>

							                <?php if($paging->next): ?>
							                    <li><a href="<?=site_url("webmin_sinyal/index/$paging->next/$o") ?>">Next</a></li>
							                <?php endif; ?>
							                <?php if($paging->end_link): ?>
							                    <li><a href="<?=site_url("webmin_sinyal/index/$paging->c_end_link/$o") ?>">Last</a></li>
							                <?php endif; ?>
										</ul>
									</div>
									<?php endif;?>
